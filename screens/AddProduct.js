...
export default function AddProduct() {
  ...
  const uploadImageToSupabase = async (uri) => {
    try {
      setUploading(true); // 1. Start Spinning!
      ...
      setUploading(false); // 2. Stop Spinning!
    }
  };

  const pickImage = async (setFieldValue) => {
    // ... (Existing pickImage logic is fine) ...
      ...
      >
        {({ handleChange, handleBlur, handleSubmit, setFieldValue, values, errors, touched, isValid, dirty }) => (
          <View style={styles.formContainer}>
            ...
            {/* ... Inputs for Title, Description, Price remain the same ... */}
            ...
            <View style={styles.inputGroup}>
              ...
              <TouchableOpacity onPress={() => { pickImage(setFieldValue); handleBlur('image'); }}>
                {values.image ? (
                  ...
                  <View style={[styles.imagePlaceholder, touched.image && errors.image ? styles.inputError : null]}>
            ...
            {/* === THIS IS THE FIXED BUTTON PART === */}
            <TouchableOpacity
              style={[
                styles.button,
                (!isValid || !dirty || uploading) ? styles.buttonDisabled : null
              ]}
              onPress={handleSubmit}
              disabled={!isValid || !dirty || uploading}
            >
              {uploading ? (
                // 3. SHOW SPINNER IF UPLOADING
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 24, fontWeight: 'bold', marginBottom: 20, color: '#333', textAlign: 'center' },
  formContainer: { backgroundColor: '#fff', padding: 20, borderRadius: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  ...
  errorText: { color: '#ff4444', fontSize: 12, marginTop: 5 },
  imagePlaceholder: { height: 150, backgroundColor: '#f0f0f0', borderRadius: 8, borderWidth: 2, borderColor: '#ddd', borderStyle: 'dashed', justifyContent: 'center', alignItems: 'center' },
  imagePlaceholderText: { fontSize: 16, color: '#666', fontWeight: '600' },
  imagePlaceholderSubText: { fontSize: 12, color: '#999', marginTop: 5 },
